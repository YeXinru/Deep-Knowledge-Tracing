...
def read_file(dataset_path):
    seqs_by_student = {}
    num_skills = 0
    dic={} # problems sparse ,transform 1346... into 0123...
    a=0
    with open(dataset_path, 'r') as f:
        for line in f:
            fields = line.strip().split()
            student, problem, is_correct, time = int(fields[0]), int(fields[1]), int(fields[2]), int(fields[3])
            if problem in dic.keys():
                ...
                a = a+1
            num_skills = max(num_skills, problem)
            seqs_by_student[student] = seqs_by_student.get(student, []) + [[problem, is_correct, time]]
            for i in seqs_by_student:
                seqs_by_student[i][0][2]=0 # the first problem of each student, time=0
    return seqs_by_student, num_skills + 1
...
def format_data(seqs, batch_size, num_skills):
    ...
    #add time into input_x
    x = pad_sequences(np.array([[([j[0] + num_skills * j[1],j[2]]) for j in i[:-1]]
                                for i in seqs_in]), maxlen=max_len, padding='post', value=-1)
    input_x = np.array([[np.append(np.array(j[1]), num_to_one_hot(j[0], num_skills * 2))
