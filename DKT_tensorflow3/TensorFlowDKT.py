...
from yxrBasicLSTMCell import yxrBasicLSTMCell
...
class TensorFlowDKT(object):
    def __init__(self, config):
        ...
        self.input_size = input_size = config["input_size"]  # 输入层节点数，等于题库数目*2
        self.batch_size = batch_size = config["batch_size"]
        self.keep_prob_value = config["keep_prob"]


        # max seq length of current batch
        self.max_steps = tf.placeholder(tf.int32)
        self.input_data = tf.placeholder(tf.float32, [batch_size, None, input_size+1])  # 答题信息
        ...
        for idx, hidden_size in enumerate(hidden_neurons):
            ...
            lstm_layer = yxrBasicLSTMCell(num_units=hidden_size, state_is_tuple=True,num_skillls=self.num_skills)
        ...
        # output layer
        print ("w_matrix----------------output_b")
        output_w = tf.get_variable("W", [hidden_neurons[-1], num_skills])
        output_b = tf.get_variable("b", [num_skills])
        self.output_w = output_w
        self.output_b = output_b

        self.state_series = tf.reshape(
            state_series, [batch_size * self.max_steps, hidden_neurons[-1]])

        self.logits = tf.matmul(self.state_series, output_w) + output_b

        self.mat_logits = tf.reshape(
            self.logits, [batch_size, self.max_steps, num_skills])
        ...
        flat_base_target_index = tf.range(
            batch_size * self.max_steps) * num_skills
        ...
        self.pred = tf.sigmoid(tf.reshape(
            flat_target_logits, [batch_size, self.max_steps]))
        ...
        self.grads, _ = tf.clip_by_global_norm(
            tf.gradients(self.loss, trainable_vars), 4)
    ...
    def step(self, sess, input_x, target_id, target_correctness, sequence_len, is_train):
        # max_steps 最长做题序列
        # sequence_len 做题序列[  0,   6, 120,  23,  14,  26,  17,  19,  13,   3]
        ...
        if is_train:
            ...
            train_loss, _, _ = sess.run(
                [self.loss, self.train_op, self.current_state], input_feed)
            return train_loss
        else:
            input_feed[self.keep_prob] = 1
            bin_pred, pred, pred_all = sess.run(
                [self.binary_pred, self.pred, self.pred_all], input_feed)
