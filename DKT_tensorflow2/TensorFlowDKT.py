...
class TensorFlowDKT(object):
    def __init__(self, config):
        ...
        self.input_data = tf.placeholder(
            tf.float32, [batch_size, None, input_size])  # 答题信息
        ...
        self.target_correctness = tf.placeholder(
            tf.float32, [batch_size, None])

        # create rnn cell
        hidden_layers = []
        for idx, hidden_size in enumerate(hidden_neurons):
            lstm_layer = tf.contrib.rnn.BasicLSTMCell(
                num_units=hidden_size, state_is_tuple=True)
            hidden_layer = tf.contrib.rnn.DropoutWrapper(cell=lstm_layer,
                                                         output_keep_prob=self.keep_prob)
            hidden_layers.append(hidden_layer)
        self.hidden_cell = tf.contrib.rnn.MultiRNNCell(
            cells=hidden_layers, state_is_tuple=True)
